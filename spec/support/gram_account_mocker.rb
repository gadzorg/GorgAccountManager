class GramAccountMocker
  DEFAULT_GRAM_ACCOUNT = {
    ...
    "alias" => %w[alexandre.narbonne.2011 84189 84189J],
    ...
    "description" =>
      "Agoram inscription - via module register - creation 2015-06-04 11:32:48",
    "entities" => %w[comptes gram],
    "is_gadz" => "true",
  }
  ...
  def self.for(hash = {})
    new(DEFAULT_GRAM_ACCOUNT.merge(hash.stringify_keys))
  end

  def mock_get_request
    ActiveResource::HttpMock.respond_to({}, false) do |mock|
      mock.get "/api/v2/accounts/#{uuid}.json",
               {
  ...
  def mock_search_request_for(param, value = nil, response = [attributes])
    ...
    ActiveResource::HttpMock.respond_to({}, false) do |mock|
      mock.get "/api/v2/accounts.json?#{param.to_s}=#{value}",
               {
                 "Authorization" => authorization_header,
                 "Accept" => "application/json",
               },
